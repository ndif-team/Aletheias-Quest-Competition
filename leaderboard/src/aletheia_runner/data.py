...
def prepare_inputs(config: RunnerConfig) -> tuple[DataLayout, bool]:
    """Build the dataset Arrow cache (in-process). Idempotent via a marker.

    Returns ``(layout, cache_hit)`` where ``cache_hit`` is True when the cache
    was already up-to-date and no download was performed.
    """
